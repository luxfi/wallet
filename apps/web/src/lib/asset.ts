 ...
 * rows; Swap + Bridge re-use this `Asset` shape verbatim so the same data
 * the portfolio renders flows into the token pickers. When Foundation merges
 * its asset module we keep this file as the stable type contract — Foundation
 * can re-export from here, or we collapse into one module (one obvious way).
 */
